...
fn ls_tree(cmd: LsTreeCommand) -> Result<(), Box<dyn std::error::Error>> {
    if cmd.name_only {
        let tree_sha = cmd.treeish;
        let object_path = format!(".git/objects/{}/{}", &tree_sha[..2], &tree_sha[2..]);
        ...
        let content = String::from_utf8_lossy(&decompressed_content);
        let mut entries = BTreeSet::new();
        
        for line in content.lines().skip(1) {  // Skip the first line (object header)
            let parts: Vec<&str> = line.split_whitespace().collect();
            if parts.len() >= 4 {
                entries.insert(parts[3].to_string());
            }
        }
        ...
        // Existing implementation for non --name-only option
...
fn create_tree_object(
    entries: &Vec<(&str, PathBuf, String)>,
